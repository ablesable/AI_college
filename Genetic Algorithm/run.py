...
def mutation(survivors, mutation_probability):
    mutation_count = 0
    for idx, person in enumerate(survivors): #for nonprimitive types. For accessing list which we iterate
        ...
        if(random_value > mutation_probability):
            ...
            list_bit_person = list(bin(person)) #making a list from a string for iteration. Forming '0bxxxxxxx'
    ...
    for i in range(0, len(survivors),2):
        ...
        try:
            ...
            if(random_value > crossing_probability): #will not be crossing
                ...
                lottery = random.randint(1, genetic_len_one-1) #crossing genomes lottery (between second and penultimate element)
